...
- So we are using CORS (Cross-Origin Resource Sharing), which is the consent mechanism. I basically add a header to my backend server that tells the browser "this origin is allowed to read my responses". So basically I am allowing localhost:4200 (angular) to read my localhost:8000 (fastAPI) responses. allow_origins=["http://localhost:4200"], is the specific line of code that does this.
...
- I've learnt that building is a very iterative process. Unless you know EXACTLY everything and the full architecture of what you want to build (which is never the case, things will pop up/change), you can't implement it perfectly from the start, and that's okay. It's about iterating and changing/adapting as you go. For example I start with localStorage, I then advance to a backend later on. Or I start with HTTP calls that lack error handling, I add the error handling later on. That is how you build an app, rather than trying to go all the way to god tier app/security etc in one specific domain, build an MVP, get your different areas layed out in a basic way, then from there start to build it up.
- Sometimes you will delve deeper into something though, for example up to this point I have been building the backend scaffolding all for my tasks (todolist) component. There is no point doing this for habit & journal. It is better to go all the way through with task first, then once that is complete, I can then do the same for habit & journal, that just makes more sense as we already have the architecture layed out for us.
- Now that we have fully set up our tasks (to-do list component) architecture. We will now do the same for Journal & Habits. We will now build from back-to-front: database, then endpoints, then tests, then frontend - so every later is provably working before the next one leans on it.